...
- 发布状态：canonical ACR run 已从 release commit 成功构建并推送 immutable backend/frontend images；ECS 已按 digest 部署，Alembic 已验证为 `0012_day17_token_version (head)`，公开 `/api/health`、`/api/ready` 和首页均为 200，JWT 已在稳定后轮换。旧 `ops/acr-build-v3.6` workflow 已标记废弃，不得使用。2026-09-12 受控生产浏览器 smoke 已创建并使用明确标识的 User A/B 测试账号：A Local 任务与 B AI 任务均完成，SSE 任务页进入 completed，A/B 反向任务 URL 均未泄露资源，A/B private template 列表均正确隔离，A 在线预览可见，浏览器 Console error/warn=0。当前浏览器/终端工具无法独立导出认证请求的原始 response headers 或下载事件，因此 `X-Request-ID`、CORS 响应头、artifact 直连下载及 artifact/SSE 直连拒绝尚未形成可审计证据；最终 `PUBLIC BETA READY` 门禁仍为 **NOT READY**。ECS 数据库、PostgreSQL volume 和现有数据 bind mounts 未被删除或替换。
