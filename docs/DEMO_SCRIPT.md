...
> 这个项目是一个基于 FastAPI 和 Next.js 的 PaperForge 学术文档可信智能处理 Agent。用户上传 DOCX 后，系统会经过文档建模、规则检测、计划执行、局部修改、重读验证、内容风险策略、人工确认和证据追踪，并提供在线预览和下载。当前版本重点是可验证执行，不承诺深度语义润色。
