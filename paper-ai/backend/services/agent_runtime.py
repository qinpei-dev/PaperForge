...
def run_runtime(*, source: Path, output_factory: Callable[[str], Path], template_path: Path | None, model: Any, rules: list[Any], plan: ExecutionPlan, before_analysis: dict[str, Any], verifier: Callable[..., Any] = verify_output) -> dict[str, Any]:
    ...
    while True:
        ...
        verification["conflict_summary"] = execution.conflict_summary
        verification["unsupported_evidence"] = execution.unsupported_evidence
    ...
    final = history[-1]
    verification_summary = final["verification"].get("verification_summary", {})
    provenance = {"changes": final["verification"].get("provenance_changes", execution.changes), "summary": {"planned_steps": len(current_plan.steps), "executed_steps": len(execution.executed_step_ids), "unsupported_steps": len(execution.unsupported_step_ids), "change_count": len(execution.changes), "verification": verification_summary, "conflicts": execution.conflict_summary.get("conflicts", 0)}}
